...
- Render report: `render-report.json` records device `Apple M5`, `commandBufferStatus` `completed`, `presentedTestPattern` true, sampled BGRA8 pixel `[0, 0, 255, 255]`, and `renderDurationNanoseconds`.
...
- A GUI screenshot was not captured because this was a noninteractive orchestration run and a full-screen desktop capture would risk committing unrelated local screen content. The practical visual evidence is the app shell `MTKView` path plus the Metal hardware render report.
